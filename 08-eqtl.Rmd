...
```{bash eval=F}
# change the path below to where you saved "bigcare-main" folder
## run in R Terminal
#mydir="/path/to/bigcare-main/Data/case_data"
mydir="./Data/eqtl"
cd $mydir
```
...
```{r eval=F}
# R codes- run in R Console
#setwd("/path/to/bigcare-main/Data/case_data")
setwd("./Data/eqtl")
mydir<-getwd()
```
...
- `gexp.txt`: rows are genes are and columns are 
...
head(covar)
```
Need to recode categorical variables as 0,1
```{r}
covar_recode<-covar                                                                  
...
Further we will explore the use of principle components as covariates in linear models of gene expression to account for unknown sources of variation from genotype. 
...
Matrix eQTL can distinguish local (cis-) and distant (trans-) eQTLs and perform separate correction for multiple comparisons for those groups. The second sample code shows how to run such analysis. The main Matrix eQTL function Matrix_eQTL_main requires several extra parameters for cis/trans analysis:
