...
The analysis consists of two parts. One part is gene analysis, which examine the assiciation between each gene and phenotype using a multiple linear regression model.

To perform the gene-set analysis, for each gene $g$, we have the gene p-value $p_g$ computed from gene analysis. We first transform the p-values $p_g$ to Z-value:
...
The MAGMA program can be downloaded from [MAGMA website](https://cncr.nl/research/magma/) and can be installed under multiple systems. You can check their manual [HERE](https://ctg.cncr.nl/software/MAGMA/doc/manual_v1.09.pdf).
...
## run in R Terminal
mydir="/path/to/cshl_2024/Data/magma"
cd $mydir
```

```{r eval=F}
# R codes- run in R Console
setwd("/path/to/cshl_2024/Data/magma")
mydir<-getwd()
```
