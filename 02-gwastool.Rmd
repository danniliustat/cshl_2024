...
To generate PC plot, we will read in the eigenvectors from **younameit.eigenvec** and check the first 2 PCs.

```{r}
# R codes- run in R Console
# read in eigenvectors
pca <- read.table("./Data/PC/younameit.eigenvec", header=F)
...
# plot pca
pc12 <- ggplot(pca, aes(PC1, PC2)) + geom_point(size = 3) +
            scale_colour_manual(values = c("red", "blue")) +
            coord_equal() + theme_light() +
            xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) +    
            ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))
pc34 <- ggplot(pca, aes(PC3, PC4)) + geom_point(size = 3) +
            scale_colour_manual(values = c("red", "blue")) +
            coord_equal() + theme_light() +
...
Usually, we use a cut-off of eigenvalue > 1. In this plot, this criteria results in an unreasonable high number of factors, because the eigenvalue for PC 50 is still > 1.
